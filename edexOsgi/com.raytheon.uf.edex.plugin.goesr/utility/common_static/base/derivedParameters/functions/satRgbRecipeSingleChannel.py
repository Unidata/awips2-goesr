'''
   <!-- 
    ...
    -->
<!-- TOWRdocs Header
  Derived Parameter Python file for implementing single channel RGB recipes.
-->
<!-- TOWRdocs Description
  This method implements single channel RGB recipes outlined in EUMETSAT's "Best
  Practices for RGB Displays of Multispectral Imagery" document 
  (http://oiswww.eumetsat.int/~idds/html/doc/best_practices.pdf). One array
  of satellite data is passed to this method along with various recipe options 
  outlined below. An array of display values is returned that corresponds to a 
  single color component of an RGB product.
-->
<!-- TOWRdocs Status
  This is a new derived parameters file.
  It is used to implement single channel RGB recipes.
-->
<!-- TOWRdocs POC
  Kevin M. McGrath
-->
...
INPUT PARAMETERS:
 	...
 	@param calibratedValueOffset:
		The amount added to the calibrated satellite data values.
...
def execute(physicalElement, calNum, calibratedValueOffset, minCalibratedValue, maxCalibratedValue, gamma, invert):
    
	#########################
	# Create calibrated array.
	if (int(calNum) == 0):
		# No need to calibrate the data.
		a_calibrated	= physicalElement
	else:
		...
		a_calibrated	=   calMethodToCall(physicalElement)

	#########################
	# Add calibratoin offset value.
	a_calibrated	= a_calibrated + calibratedValueOffset

	#########################
	# Clip the calibrated values
	a_calibrated_clipped = np.clip(a_calibrated, minCalibratedValue, maxCalibratedValue)

	#########################
	# Generate display values by implement EUMETSAT recipe.
	dispValue_float  = 255.*(np.power( (a_calibrated_clipped - minCalibratedValue)/(maxCalibratedValue - minCalibratedValue), (1./gamma)))
	...
	dispValue_byte[(physicalElement == 0)] = 0
